<template>
    <div class="calendar">
      ...
      <div class="day">
        <div class="item" :class="{'item-selected':item.isToday,'item-today':item.date === currentDate}" v-for="(item,index) in list" :key="index" @click="clickDay(item,index)">
          <text class="text" :class="{'text-selected':item.isToday}" v-if="isShowOtherday || ( index + 1 >= item.id && index - item.id  < 7)">{{item.id}}</text>
          <image class="icon" v-if="item.sign"  :src="item.isToday ? require('assets/right_white.png') : require('assets/right_gold.png')"/>
        </div>

        <!-- <div class="item" :class="{'item-selected':item.isToday}" v-for="(item,index) in list" :key="index">
          <text class="text"  v-if="isShowOtherday || ( index + 1 >= item.id && index - item.id  < 7)">{{item.id}}</text>
          <image class="icon" v-if="item.sign"  :src="require('assets/right_gold.png')"/>
        </div> -->
      </div>
    </div>
</template>
